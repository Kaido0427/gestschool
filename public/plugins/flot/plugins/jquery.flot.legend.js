...
(function($) {
    ...
    function insertLegend(plot, options, placeholder, legendEntries) {
        ...
        // Save the legend données in legend options
        var données = options.legend.legendEntries = legendEntries,
        ...
        // Generate html for icons and labels from a list of données
        for (i = 0; i < données.length; ++i) {
            let columnIndex = i % options.legend.noColumns;
            entry = données[i];
            iconHtml = '';
        ...
        var legendEl,
            height = Math.ceil(données.length / options.legend.noColumns) * 1.6;
    ...
    // Generate a list of legend données in their final order
    ...
    // Compare two lists of legend données
